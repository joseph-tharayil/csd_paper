  ...
  if (nrn_point_prop_) {
   ...
   _ppvar = nrn_prop_datum_alloc(_mechtype, 12, _prop);
    ...
    assert(_nrn_mechanism_get_num_vars(_prop) == 54);
 	...
 	gmax0_AMPA = _parm_default[1]; /* 1 */
 	gmax_d_AMPA = _parm_default[2]; /* 1 */
 	gmax_p_AMPA = _parm_default[3]; /* 2 */
 	gmax_NMDA = _parm_default[4]; /* 0.55 */
 	Use = _parm_default[5]; /* 0.5 */
 	Dep = _parm_default[6]; /* 100 */
 	Fac = _parm_default[7]; /* 10 */
 	Nrrp = _parm_default[8]; /* 1 */
 	Use_d = _parm_default[9]; /* 0.2 */
 	Use_p = _parm_default[10]; /* 0.8 */
 	volume_CR = _parm_default[11]; /* 0.087 */
 	theta_d_GB = _parm_default[12]; /* 0.006 */
 	theta_p_GB = _parm_default[13]; /* 0.001 */
 	rho0_GB = _parm_default[14]; /* 0 */
 	synapseID = _parm_default[15]; /* 0 */
 	verbose = _parm_default[16]; /* 0 */
 	selected_for_report = _parm_default[17]; /* 0 */
 	conductance = _parm_default[18]; /* 0 */
  }
 	 assert(_nrn_mechanism_get_num_vars(_prop) == 54);
 	_nrn_mechanism_access_dparam(_prop) = _ppvar;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 6;
    ...
    hoc_register_var(hoc_scdoub, hoc_vdoub, hoc_intfunc);
 	ivoc_help("help ?1 GluSynapse /mnt/mydata/Documents/csdRevisions/csd_paper/hypothesisTesting/mods_dir/GluSynapse.mod\n");
...
for (size_t _iml = 0; _iml < _cntml; ++_iml) {
 ...
 v=_v;
{
 {   euler_thread(11, _slist1, _dlist1, neuron::scopmath::row_view{_ml, _iml}, state, _ml, _iml, _ppvar, _thread, _globals, _nt);
     if (secondorder) {
    int _i;
    for (_i = 0; _i < 11; ++_i) {
...
static void register_nmodl_text_and_filename(int mech_type) {
    const char* nmodl_filename = "/mnt/mydata/Documents/csdRevisions/csd_paper/hypothesisTesting/mods_dir/GluSynapse.mod";
