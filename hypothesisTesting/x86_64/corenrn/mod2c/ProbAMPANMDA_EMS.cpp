...
namespace coreneuron {
    ...
    struct ProbAMPANMDA_EMS_Store {
        ...
        double gmax{0.001};
        int slist1[4]{22, 23, 24, 25};
        int dlist1[4]{30, 31, 32, 33};
    };
    ...
    static DoubScal hoc_scalar_double[] = {
        {"tau_r_AMPA_ProbAMPANMDA_EMS", &ProbAMPANMDA_EMS_global.tau_r_AMPA},
        {"mg_ProbAMPANMDA_EMS", &ProbAMPANMDA_EMS_global.mg},
        ...
        {"tau_r_NMDA_ProbAMPANMDA_EMS", &ProbAMPANMDA_EMS_global.tau_r_NMDA},
        {"tau_d_NMDA_ProbAMPANMDA_EMS", &ProbAMPANMDA_EMS_global.tau_d_NMDA},
    ...
    static inline void setup_instance(NrnThread* nt, Memb_list* ml) {
        ...
        inst->Use = ml->data+1*pnodecount;
        inst->Dep = ml->data+2*pnodecount;
        inst->Fac = ml->data+3*pnodecount;
        inst->u0 = ml->data+4*pnodecount;
        inst->Nrrp = ml->data+5*pnodecount;
        inst->synapseID = ml->data+6*pnodecount;
        inst->verboseLevel = ml->data+7*pnodecount;
        inst->selected_for_report = ml->data+8*pnodecount;
        inst->NMDA_ratio = ml->data+9*pnodecount;
        inst->conductance = ml->data+10*pnodecount;
        inst->i = ml->data+11*pnodecount;
        inst->i_AMPA = ml->data+12*pnodecount;
        inst->i_NMDA = ml->data+13*pnodecount;
        inst->g_AMPA = ml->data+14*pnodecount;
        inst->g_NMDA = ml->data+15*pnodecount;
        inst->g = ml->data+16*pnodecount;
        inst->unoccupied = ml->data+17*pnodecount;
        inst->occupied = ml->data+18*pnodecount;
        inst->tsyn = ml->data+19*pnodecount;
        inst->u = ml->data+20*pnodecount;
        inst->next_delay = ml->data+21*pnodecount;
        inst->A_AMPA = ml->data+22*pnodecount;
        inst->B_AMPA = ml->data+23*pnodecount;
        inst->A_NMDA = ml->data+24*pnodecount;
        inst->B_NMDA = ml->data+25*pnodecount;
        inst->factor_AMPA = ml->data+26*pnodecount;
        inst->factor_NMDA = ml->data+27*pnodecount;
        inst->mggate = ml->data+28*pnodecount;
        inst->usingR123 = ml->data+29*pnodecount;
        inst->DA_AMPA = ml->data+30*pnodecount;
        inst->DB_AMPA = ml->data+31*pnodecount;
        inst->DA_NMDA = ml->data+32*pnodecount;
        inst->DB_NMDA = ml->data+33*pnodecount;
        inst->v_unused = ml->data+34*pnodecount;
        inst->g_unused = ml->data+35*pnodecount;
        inst->tsave = ml->data+36*pnodecount;
    ...
    inline int toggleVerbose_ProbAMPANMDA_EMS(int id, int pnodecount, ProbAMPANMDA_EMS_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v);
}
...
namespace coreneuron {
    ...
    static inline void net_receive_kernel_ProbAMPANMDA_EMS(double t, Point_process* pnt, ProbAMPANMDA_EMS_Instance* inst, NrnThread* nt, Memb_list* ml, int weight_index, double flag) {
        ...
        inst->tsave[id] = t;
        {
            ...
            if (ves > 0.0) {
                ...
                if (inst->verboseLevel[id] > 0.0) {
                    printf("[Syn %.0f] Release! t = %g, vals: %g %g %g %g\n", inst->synapseID[id], t, inst->A_AMPA[id], (*weight_AMPA), inst->factor_AMPA[id], (*weight));
                }
            } else {
                if (inst->verboseLevel[id] > 0.0) {
    ...
    void nrn_init_ProbAMPANMDA_EMS(NrnThread* nt, Memb_list* ml, int type) {
        ...
        if (_nrn_skip_initmodel == 0) {
            ...
            for (int id = 0; id < nodecount; id++) {
                ...
                inst->A_NMDA[id] = 0.0;
                inst->B_NMDA[id] = 0.0;
                tp_AMPA = (inst->global->tau_r_AMPA * inst->tau_d_AMPA[id]) / (inst->tau_d_AMPA[id] - inst->global->tau_r_AMPA) * log(inst->tau_d_AMPA[id] / inst->global->tau_r_AMPA);
                tp_NMDA = (inst->global->tau_r_NMDA * inst->global->tau_d_NMDA) / (inst->global->tau_d_NMDA - inst->global->tau_r_NMDA) * log(inst->global->tau_d_NMDA / inst->global->tau_r_NMDA);
                inst->factor_AMPA[id] =  -exp( -tp_AMPA / inst->global->tau_r_AMPA) + exp( -tp_AMPA / inst->tau_d_AMPA[id]);
                inst->factor_AMPA[id] = 1.0 / inst->factor_AMPA[id];
                inst->factor_NMDA[id] =  -exp( -tp_NMDA / inst->global->tau_r_NMDA) + exp( -tp_NMDA / inst->global->tau_d_NMDA);
    ...
    inline double nrn_current_ProbAMPANMDA_EMS(int id, int pnodecount, ProbAMPANMDA_EMS_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v) {
        double current = 0.0;
        inst->mggate[id] = 1.0 / (1.0 + exp(inst->global->slope_mg *  -(v)) * (inst->global->mg / inst->global->scale_mg));
    ...
    void nrn_state_ProbAMPANMDA_EMS(NrnThread* nt, Memb_list* ml, int type) {
        ...
        for (int id = 0; id < nodecount; id++) {
            ...
            inst->v_unused[id] = v;
            #endif
            inst->A_AMPA[id] = inst->A_AMPA[id] + (1.0 - exp(nt->_dt * (( -1.0) / inst->global->tau_r_AMPA))) * ( -(0.0) / (( -1.0) / inst->global->tau_r_AMPA) - inst->A_AMPA[id]);
            inst->B_AMPA[id] = inst->B_AMPA[id] + (1.0 - exp(nt->_dt * (( -1.0) / inst->tau_d_AMPA[id]))) * ( -(0.0) / (( -1.0) / inst->tau_d_AMPA[id]) - inst->B_AMPA[id]);
            inst->A_NMDA[id] = inst->A_NMDA[id] + (1.0 - exp(nt->_dt * (( -1.0) / inst->global->tau_r_NMDA))) * ( -(0.0) / (( -1.0) / inst->global->tau_r_NMDA) - inst->A_NMDA[id]);
            inst->B_NMDA[id] = inst->B_NMDA[id] + (1.0 - exp(nt->_dt * (( -1.0) / inst->global->tau_d_NMDA))) * ( -(0.0) / (( -1.0) / inst->global->tau_d_NMDA) - inst->B_NMDA[id]);
        }
    }
    ...
    void _ProbAMPANMDA_EMS_reg() {
        ...
        point_register_mech(mechanism_info, nrn_alloc_ProbAMPANMDA_EMS, nrn_cur_ProbAMPANMDA_EMS, nullptr, nrn_state_ProbAMPANMDA_EMS, nrn_init_ProbAMPANMDA_EMS, nrn_private_constructor_ProbAMPANMDA_EMS, nrn_private_destructor_ProbAMPANMDA_EMS, first_pointer_var_index(), nullptr, nullptr, 1);
