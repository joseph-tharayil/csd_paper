/*********************************************************
Model Name      : NaTg_somadend
Filename        : NaTg_somadend.mod
NMODL Version   : 7.7.0
Vectorized      : true
Threadsafe      : true
...
namespace coreneuron {
    ...
    static_assert(std::is_trivially_copy_constructible_v<NaTg_somadend_Store>);
    static_assert(std::is_trivially_move_constructible_v<NaTg_somadend_Store>);
    static_assert(std::is_trivially_copy_assignable_v<NaTg_somadend_Store>);
    static_assert(std::is_trivially_move_assignable_v<NaTg_somadend_Store>);
    static_assert(std::is_trivially_destructible_v<NaTg_somadend_Store>);
    NaTg_somadend_Store NaTg_somadend_global;


    /** all mechanism instance variables and global variables */
    struct NaTg_somadend_Instance  {
        ...
        NaTg_somadend_Store* global{&NaTg_somadend_global};
    };
    ...
    static inline int get_mech_type() {
        return NaTg_somadend_global.mech_type;
    }
    ...
    static void nrn_private_constructor_NaTg_somadend(NrnThread* nt, Memb_list* ml, int type) {
        assert(!ml->instance);
        assert(!ml->global_variables);
        assert(ml->global_variables_size == 0);
        auto* const inst = new NaTg_somadend_Instance{};
        assert(inst->global == &NaTg_somadend_global);
        ml->instance = inst;
        ml->global_variables = inst->global;
        ml->global_variables_size = sizeof(NaTg_somadend_Store);
    }

    // Deallocate the instance structure
    static void nrn_private_destructor_NaTg_somadend(NrnThread* nt, Memb_list* ml, int type) {
        auto* const inst = static_cast<NaTg_somadend_Instance*>(ml->instance);
        assert(inst);
        assert(inst->global);
        assert(inst->global == &NaTg_somadend_global);
        assert(inst->global == ml->global_variables);
        assert(ml->global_variables_size == sizeof(NaTg_somadend_Store));
        delete inst;
        ml->instance = nullptr;
    ...
    static void nrn_alloc_NaTg_somadend(double* data, Datum* indexes, int type) {
        // do nothing
    }


    void nrn_constructor_NaTg_somadend(NrnThread* nt, Memb_list* ml, int type) {
    ...
    void nrn_destructor_NaTg_somadend(NrnThread* nt, Memb_list* ml, int type) {
        #ifndef CORENEURON_BUILD
        int nodecount = ml->nodecount;
        int pnodecount = ml->_nodecount_padded;
        const int* node_index = ml->nodeindices;
    ...
    inline int rates_NaTg_somadend(int id, int pnodecount, NaTg_somadend_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v);
    ...
    /** initialize channel */
    void nrn_init_NaTg_somadend(NrnThread* nt, Memb_list* ml, int type) {
        int nodecount = ml->nodecount;
        int pnodecount = ml->_nodecount_padded;
        const int* node_index = ml->nodeindices;
        double* data = ml->data;
    ...
    inline double nrn_current_NaTg_somadend(int id, int pnodecount, NaTg_somadend_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v) {
        double current = 0.0;
    ...
    /** update current */
    void nrn_cur_NaTg_somadend(NrnThread* nt, Memb_list* ml, int type) {
        int nodecount = ml->nodecount;
        int pnodecount = ml->_nodecount_padded;
        const int* node_index = ml->nodeindices;
        double* data = ml->data;
        ...
        for (int id = 0; id < nodecount; id++) {
            ...
            inst->ttxo[id] = inst->ion_ttxo[indexes[3*pnodecount + id]];
            inst->ttxi[id] = inst->ion_ttxi[indexes[4*pnodecount + id]];
            double g = nrn_current_NaTg_somadend(id, pnodecount, inst, data, indexes, thread, nt, v+0.001);
            double dina = inst->ina[id];
            double rhs = nrn_current_NaTg_somadend(id, pnodecount, inst, data, indexes, thread, nt, v);
            g = (g-rhs)/0.001;
    ...
    /** update state */
    void nrn_state_NaTg_somadend(NrnThread* nt, Memb_list* ml, int type) {
        int nodecount = ml->nodecount;
        int pnodecount = ml->_nodecount_padded;
        const int* node_index = ml->nodeindices;
        double* data = ml->data;
    ...
    void _NaTg_somadend_reg() {

        int mech_type = nrn_get_mechtype("NaTg_somadend");
        NaTg_somadend_global.mech_type = mech_type;
        if (mech_type == -1) {
            return;
        }

        _nrn_layout_reg(mech_type, 0);
        register_mech(mechanism_info, nrn_alloc_NaTg_somadend, nrn_cur_NaTg_somadend, nullptr, nrn_state_NaTg_somadend, nrn_init_NaTg_somadend, nrn_private_constructor_NaTg_somadend, nrn_private_destructor_NaTg_somadend, first_pointer_var_index(), 1);
        NaTg_somadend_global.na_type = nrn_get_mechtype("na_ion");
        NaTg_somadend_global.ttx_type = nrn_get_mechtype("ttx_ion");
